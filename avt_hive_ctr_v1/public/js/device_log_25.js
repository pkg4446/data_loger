...
}else if(localStorage.getItem('device_25') === null){
    window.location.href = '/web/select_25';
}else{
    document.getElementById('divece_name').innerText = localStorage.getItem('device_25');
...
function draw_chart(date_data){
    data_button(false);
    echarts_draw(temperatures[date_data],4,false,"°C","hive_graph_temp","TP",0);
    echarts_draw(temperatures[date_data],4,false,"%","hive_graph_humi","HM",0);
    echarts_draw_heater(temperatures[date_data]);
}
////-------------------////
function data_type(index,raw){
    data_button(raw);
    const date_data = date_parser(new Date(document.getElementById('data_day').value));
    echarts_draw(temperatures[date_data],index,raw,"°C","hive_graph_temp","TP",0);
...
function data_independent(type,index,raw){
    const date_data = date_parser(new Date(document.getElementById('data_day').value));
    if(type == 0){      echarts_draw(temperatures[date_data],index,raw,"°C","hive_graph_temp","TP",0);
    }else if(type == 1){echarts_draw(temperatures[date_data],index,raw,"%","hive_graph_humi","HM",0);
    }else{              echarts_draw_heater(temperatures[date_data]);}
}
////-------------------////
function data_button(raw){
    let HTML_script = '<button class="search-btn" onclick="data_type(4,';
    let data_type   = "";
    if(raw){
        data_type   = "true";
    ...
    for (let index = 0; index < 2; index++) {
        HTML_script =   `<button class="search-btn" onclick="data_independent(${index},4,${data_type})">All</button>
                    <button class="search-btn btn-type1" onclick="data_independent(${index},0,${data_type})">히터</button>
                    <button class="search-btn btn-type2" onclick="data_independent(${index},1,${data_type})">공간</button>
                    <button class="search-btn btn-type3" onclick="data_independent(${index},2,${data_type})">봉구</button>
                    <button class="search-btn btn-type4" onclick="data_independent(${index},3,${data_type})">외부</button>`;
...
function getdata(date_now){
    ...
    fetch(window.location.protocol+"//"+window.location.host+"/hive_25/log", {
        method: 'POST',
        headers: {
...
////-------------------////
function echarts_draw_heater(draw_data) {
    const option_basic = {
        tooltip: {trigger: 'axis'},
        toolbox: {
            show: true,
    ...
    if(draw_data != undefined && draw_data.length != 0){
        for (let index = 0; index < draw_data.length; index++) {            
            option_ht.series[0].data.push((draw_data[index].WK/draw_data[index].GAP*40).toFixed(2));
            energy_use += draw_data[index].WK/90;
            option_vi.series[0].data.push(energy_use.toFixed(2));//90=3600/40
        }
    }
