...
router.post('/connect', async function(req, res) {
    ...
    if(user_data.id!=undefined && user_data.token!=undefined && user_data.dvid!=undefined && user_data.name!=undefined && user_data.name.length>0){
        ...
        if(token_check(user_data.token,user_data.id)){
            status_code = await device.connect(user_data.id,"device_25",user_data.dvid,user_data.name);
        }else{
            status_code = 401;
        }
    }
...
router.post('/disconnect', async function(req, res) {
    ...
    if(user_data.id!=undefined && user_data.token!=undefined && user_data.dvid!=undefined){
        if(token_check(user_data.token,user_data.id)){
            status_code = await device.disconnect(user_data.id,"device_25",user_data.dvid);
        }else{
            status_code = 401;
        }
    }
...
router.post('/devicerename', async function(req, res) {
    ...
    if(user_data.id!=undefined && user_data.token!=undefined && user_data.dvid!=undefined){
        ...
        if(token_check(user_data.token,user_data.id)){
            status_code = 200;
            const list   = file_system.fileRead(path_user,"device_25.csv").split("\r\n");
            let new_list = "";
            ...
            file_system.fileMK(path_user,new_list,"device_25.csv");
        }else{
            status_code = 401;
        }
    }
...
router.post('/heater', async function(req, res) {
    ...
    const user_data = req.body;
    if(user_data.id!=undefined && user_data.token!=undefined && user_data.dvid!=undefined){
        const   path_device = path_data.device("device_25")+"/"+user_data.dvid;
        if(token_check(user_data.token,user_data.id)){
...
router.post('/list', async function(req, res) {
    ...
    if(user_data.id!=undefined && user_data.token!=undefined){
        const   path_user   = path_data.user()+"/"+user_data.id;
        if(token_check(user_data.token,user_data.id)){
            if(file_system.check(path_user+"/device_25.csv")){
                status_code = 200;
                response    = file_system.fileRead(path_user,"device_25.csv");
            }else{
                status_code = 403;
                response    = "device";
            }
...
router.post('/list_arrange', async function(req, res) {
    ...
    if(user_data.id!=undefined && user_data.token!=undefined){
        ...
        if(token_check(user_data.token,user_data.id)){
            ...
            if(file_system.fileRead(path_user,"device_25.csv") != device) file_system.fileMK(path_user,device,"device_25.csv");
        }else{
            status_code = 406;
        }
    }
